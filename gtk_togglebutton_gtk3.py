...
class Example(Gtk.Window):
    def __init__(self, app):
        Gtk.Window.__init__(self, application=app, title="トグルボタン")
        # Box layout
        box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL)
        self.set_child(box)
        #
        cb1 = Gtk.ToggleButton(label="トグルボタンＡ")
        cb1.connect("toggled", self.on_button_toggled, "Ａ")
        box.append(cb1)
        #
        cb2 = Gtk.ToggleButton(label="トグルボタンＢ")
        cb2.connect("toggled", self.on_button_toggled, "Ｂ")
        box.append(cb2)
        #
        cb3 = Gtk.ToggleButton(label="トグルボタンＣ")
